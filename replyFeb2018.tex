...
% Our work focuses on leveraging existing temperature sensoring
% infrastructure for occupency detection in a buiding by 
%  applying machine learning techniques, such as deep
% recurrent neural networks.  The main benefit of the exsting method is
% no need of additional This does not require explicitly
...
%(TODO: need to re-answer this question)!--answered
Answer: Since SVR has great generalization ability and can guarantee to find the global minimum,
SVR can perform well for time series analysis for occupancy detection. A recurrent
neural network (RNN) can also be used for a time series prediction problem as the data
configuration for RNN includes time steps. The most important features related to occupancy
behavior in a building are applied in the machine learning methods. These features covers 
all major thermal-related aspects to be coupled with the human activity. As a result, 
occupancy behavior can be determined theoretically by using the machine learning methods
with time series analysis.  

Besides, comparing to the referencing result, the occupation detection
