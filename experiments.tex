...
performance. Here we also want to highlight, most of the time,
the best choice of the parameter set is usually difficult to be theoretically proved,
therefore confirmed parameters often are determined by
...
means that this model is over-fitting on the training data. The accuracy of training
data implies that the model has a potential under-fitting. In this
...
while the second set requires more effort. The first set of feature only
...
suggested to choose the most efficient approach based on the specific situation.
...
people, which may have fraction part as it represents average people count over a short
...
relatively easy to be acquired or calculated. While other factors, such as
...
In configuration I, one-layer network suffers from under-fitting
...
In configuration II, we provide more information (HVAC powers)
...
methods agree well with the original curves. Furthermore, we observe
there is a small fluctuation in maximum error when the SVR model is
...
changed. In comparison, maximum error of the RNN model is relatively more
sensitive. The maximum error may double, triple, or rapidly diminishes
when the features of the model is changed. This phenomenon is also
observed in the average error. The numeric value of average error swings
